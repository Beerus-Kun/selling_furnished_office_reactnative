...
export default function ChangeCategory({route}) {
    ...
    const addCoupon = async () => {
        if (type && name) {
            const res = await _function.putAPI(`product/category/${route.params.item.id_category}?type=${type}&name=${name}`, {}, token)
            if (res.code == 200) {
                alert("Thành công", "Bạn thay đổi loại sản phẩm thành công")
                // setName('')
            }
        } else {
            var noti = ''
